...
export function ReportsSidebarItem({ isActive }: { isActive: boolean }) {
    const location = useLocation();
    const { isCollapsed } = useSidebar();

    const reportsLinks = [
        { icon: BarChart3, label: "Reports", path: "/reports" },
        { icon: Wallet, label: "Finance", path: "/reports/finance" },
    ];
                ...
                <Tooltip delayDuration={300}>
                    <TooltipTrigger asChild>
                        <DropdownMenuTrigger asChild>
                            ...
                            >
                                <BarChart3 className={cn("h-[16px] w-[16px]", isActive ? "text-[#DA291C]" : "text-[#888888]")} />
                            </button>
                        </DropdownMenuTrigger>
                    </TooltipTrigger>
            ...
            <AccordionItem value="reports" className="border-0">
                ...
                >
                    <div className="flex items-center gap-3">
                        <BarChart3 className={cn("h-[16px] w-[16px]", isActive ? "text-[#DA291C]" : "text-[#888888] group-hover:text-white")} />
                        Reports
                    </div>
                </AccordionTrigger>
